...
def calculate_shaved_payout(policy_dna_shaving: dict, modern_treatments: dict, non_payable_items: list, 
    ...
    # Slugify modern treatments (remove spaces/case) for fuzzy matching, only include covered treatments
    normalized_modern_map = {"".join(k.lower().split()): v["limit"] for k, v in modern_treatments.items() if v.get("covered", False)}
    ...
    for item in hospital_bill:
        ...
        for slug, cap_str in normalized_modern_map.items():
            if slug in item_slug or item_slug in slug:
                if cap_str != "Up to Sum Insured":
                    try:
                        ...
                        pass  # If not a number, assume no cap
                break
